...
function Login() {
  ...
  const loginHandler = async (e) => {
    ...
    if (filtered.length > 0) {
      ...
      cookies.set("data", JSON.stringify(filtered[0]), {
        path: "/",
        maxAge: 14 * 24 * 60 * 60,
      });
  ...
  return (
    <div className="container mr-auto ml-auto">
      ...
      <div className="relative p-4 sm:p-8 mx-5 sm:mx-auto max-w-2xl bg-gray-50 rounded-xl shadow-2x1 items-center sm:max-w-md">
        <h2 className=" text-3xl font-bold mb-2 text-indigo-600 text-center">
          UTS KIT
        </h2>
        <div className="italic font-light font-sans text-center">
          Dapat digunakan oleh seluruh mahasiswa non-asrama 🎉{" "}
        </div>
        <form className="mt-4" onSubmit={loginHandler}>
          <div>
            <label
              className="block mb-2 font-bold text-grey text-center"
              htmlFor="npm"
            >
              NPM
            </label>
          ...
          <div className="mt-8 text-center text-gray-500 text-sm">
            <button
              type="submit"
              className="block w-full mb-2 bg-blue-500 p-2 rounded text-white font-bold hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed text-md"
              id="btnLogin"
            >
              <FontAwesomeIcon icon={faSignInAlt} /> Sign in
            </button>
            This site is protected by{" "}
            <span className="font-medium">reCAPTCHA</span> and the Google{" "}
            <span className="font-medium">
              <a href="https://policies.google.com/privacy">Privacy Policy</a>
            </span>{" "}
            and{" "}
            <span className="font-medium">
              <a href="https://policies.google.com/privacy">Terms of Service</a>
            </span>{" "}
            apply.
          </div>
        </form>
